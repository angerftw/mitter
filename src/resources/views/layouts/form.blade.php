<div class="row">
	...
	@if($isDeleted)
		<form method="POST" action="{{$model->getStoreAction()}}" accept-charset="UTF-8">
	...
	@elseif(isset($id))
		<form method="POST" action="{{$model->getDeleteAction()}}" accept-charset="UTF-8">
...
<form method="POST" action="{{$model->getStoreAction()}}" accept-charset="UTF-8" parsley-validate="parsley-validate" novalidate="novalidate" enctype="multipart/form-data">
	<input name="_token" type="hidden" value="{{csrf_token()}}">
	@if (isset($id))
